...
#                 result = torch.max(logit,1)[1].view(target.size())
                ...
                sys.stdout.write('\rBatch[{}] - loss: {:.6f} acc: {:.4f}%({}/{}) nonOther acc: {:.4f}%（{}/{}）'
                 .format(steps,loss.data.item()/len(train_iter.dataset),accuracy,corrects,batch.batch_size,nonOtherAcc,nonOtherCorrects,nonOther))
